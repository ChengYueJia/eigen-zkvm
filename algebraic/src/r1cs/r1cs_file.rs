//! Porting from https://github.com/poma/zkutil/blob/master/src/r1cs_reader.rs
...
use crate::r1cs::{
    ...
    custome_gate::{CustomGates, CustomGatesUses},
    header::Header,
};
...
impl<E: ScalarEngine> R1CSFile<E> {
    ...
    pub fn from_reader<R: Read + Seek>(mut reader: R) -> Result<R1CSFile<E>> {
        ...
        if section_offsets.get(&CUSTOM_GATES_LIST).is_some() {
            ...
            custom_gates = CustomGates::read_custom_gates_list(&mut reader)?;
        }
