...
impl<E: ScalarEngine> CustomGates<E> {
    ...
    fn read_to_string<R: Read>(_reader: &mut R) -> String {
        let name_buf = vec![1u8; 1];
        let mut buf = vec![];
        loop {
    ...
    pub(crate) fn read_custom_gates_list<R: Read>(mut reader: R) -> Result<Vec<CustomGates<E>>> {
...
impl CustomGatesUses {
    ...
    pub(crate) fn read_custom_gates_uses_list<R: Read>(
        mut reader: R,
        size: u64,
